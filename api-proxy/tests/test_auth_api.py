...
AUTH_RECOVERY_MODULE_PATH = REPO_ROOT / "api-proxy" / "app_modules" / "auth_recovery.py"
...
def load_auth_recovery_module(module_name: str):
    spec = importlib.util.spec_from_file_location(
        module_name, AUTH_RECOVERY_MODULE_PATH
    )
    module = importlib.util.module_from_spec(spec)
    sys.modules[module_name] = module
    assert spec.loader is not None
...
def register_user(
    ...
    *,
    ...
    response = client.post(
        "/api/auth/register", json=register_payload(email), headers=headers or {}
    )
...
def test_duplicate_registration_uses_generic_error(client: TestClient):
    ...
    duplicate = client.post(
        "/api/auth/register", json=register_payload("dup@example.com")
    )
    assert duplicate.status_code == 409
    assert duplicate.json()["detail"] == "unable to create account"


def test_unknown_login_still_uses_password_verifier(
    api_module, client: TestClient, monkeypatch: pytest.MonkeyPatch
):
...
def test_password_reset_request_returns_generic_success_for_known_and_unknown_email(
    ...
    monkeypatch.setattr(
        api_module, "send_password_reset_email", fake_send_password_reset_email
    )
    register_user(client, email="recover@example.com")

    known = client.post(
        "/api/auth/password-reset/request", json={"email": "recover@example.com"}
    )
    unknown = client.post(
...
def test_password_reset_request_stores_only_hashed_code_and_respects_cooldown(
    ...
    monkeypatch.setattr(
        api_module, "generate_password_reset_code", lambda: next(issued_codes)
    )
    monkeypatch.setattr(api_module, "send_password_reset_email", lambda **_: None)
    register_user(client, email="cooldown@example.com")

    first = client.post(
        "/api/auth/password-reset/request", json={"email": "cooldown@example.com"}
    )
    second = client.post(
        "/api/auth/password-reset/request", json={"email": "cooldown@example.com"}
    )
...
def test_password_reset_request_cleans_up_code_when_email_send_fails(
    ...
    with pytest.raises(RuntimeError, match="SMTP is not configured"):
        client.post(
            "/api/auth/password-reset/request", json={"email": "send-fail@example.com"}
        )
...
def test_password_reset_dev_log_fallback_does_not_log_plaintext_code_or_email(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
):
    module_name = f"auth_recovery_test_{len(sys.modules)}"
    auth_recovery_module = load_auth_recovery_module(module_name)

    monkeypatch.delenv("SMTP_HOST", raising=False)
    monkeypatch.setenv("PASSWORD_RESET_DEV_LOG_FALLBACK", "true")

    auth_recovery_module.send_password_reset_email(
        to_email="sensitive@example.com",
        code="123456",
    ...
    assert "sensitive@example.com" not in output
...
def test_password_reset_confirm_replaces_password_and_invalidates_sessions(
    ...
    request_response = client.post(
        "/api/auth/password-reset/request", json={"email": "reset-success@example.com"}
    )
...
def test_password_reset_confirm_rejects_wrong_expired_and_exhausted_codes(
    ...
    request_response = client.post(
        "/api/auth/password-reset/request", json={"email": "reset-fail@example.com"}
    )
    assert request_response.status_code == 200, request_response.text
