...
function AsyncPanelFallback({ label }: { label: string }) {
  return (
    <GlassCard variant="soft" padding="md" hoverable={false} role="status" aria-live="polite" className="text-center text-sm text-slate-500 dark:text-slate-300">
      {label}
    </GlassCard>
  );
}
...
function PracticeCatalogErrorState({
  ...
  return (
    <GlassCard variant="soft" padding="lg" hoverable={false} className="border-rose-200/80 bg-rose-50/80 text-center dark:border-rose-900/40 dark:bg-rose-950/30">
...
export default function Practice() {
  ...
  const topTabs: TabItem<'preset' | 'ai'>[] = [
    { id: 'preset', label: t('题库练习') },
    { id: 'ai', label: t('AI 智能出题') },
  ];
  const difficultyFilterOptions: TabItem<'all' | 'easy' | 'medium' | 'hard'>[] = [
    { id: 'all', label: isEnglish ? 'All' : '全部' },
    { id: 'easy', label: `${getDifficultyMeta('easy').stars} ${t(getDifficultyMeta('easy').label)}` },
    { id: 'medium', label: `${getDifficultyMeta('medium').stars} ${t(getDifficultyMeta('medium').label)}` },
    { id: 'hard', label: `${getDifficultyMeta('hard').stars} ${t(getDifficultyMeta('hard').label)}` },
  ];
  const typeFilterOptions: TabItem<'all' | 'coding' | 'fillblank'>[] = [
    { id: 'all', label: isEnglish ? 'All' : '全部' },
    { id: 'coding', label: t('编程题') },
    { id: 'fillblank', label: t('填空题') },
  ];
  const completionOptions: TabItem<'all' | 'completed' | 'incomplete'>[] = [
    { id: 'all', label: isEnglish ? 'All' : '全部' },
    { id: 'incomplete', label: isEnglish ? 'Incomplete' : '未完成' },
    { id: 'completed', label: isEnglish ? 'Completed' : '已完成' },
  ];
    ...
    <div className="min-h-screen pb-8">
      ...
      <div className="mx-auto max-w-5xl px-4 sm:px-6">
        <section className="relative z-10 mb-8">
          <StatStrip
            items={[
              { value: hasLoadedExercises ? exercises.length : '…', label: isEnglish ? 'Unique Problems' : '去重后题量', tone: 'klein' },
              { value: progress.completedExercises.length, label: isEnglish ? 'Completed' : '已完成', tone: 'emerald' },
              { value: reviewQueue.length, label: isEnglish ? 'Review Queue' : '待复盘', tone: 'amber' },
              { value: recentScore, label: isEnglish ? 'Recent Avg Score' : '近期平均得分', tone: 'indigo' },
            ]}
          />
        </section>
        ...
        <div
          ...
          onMouseEnter={preloadAllExercises}
          onFocus={preloadAllExercises}
        >
          <Tabs
            items={topTabs}
            value={tab}
            ...
            variant="pill"
            size="md"
            ariaLabel={isEnglish ? 'Practice mode' : '练习模式'}
          />
        </div>
        ...
        <div className="relative z-10 space-y-8">
          ...
          <>
            {!selectedExercise && isExercisesLoading ? (
              ...
              <>
                <GlassCard variant="soft" padding="md" hoverable={false} className="mb-8">
                  <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    <div className="space-y-3">
                      <div className="flex flex-wrap items-center gap-2">
                        <span className="inline-flex items-center gap-1.5 rounded-full bg-klein-600 px-3 py-1 text-xs font-semibold text-white">
                          <svg width="10" height="10" viewBox="0 0 24 24" fill="currentColor" aria-hidden>
                            <path d="M12 3.2l2.62 5.31 5.86.85-4.24 4.13 1 5.84L12 16.77l-5.24 2.56 1-5.84L3.52 9.36l5.86-.85L12 3.2z" />
                          </svg>
                          {t('每日推荐')}
                        </span>
                        <span className="rounded-full border border-slate-200 bg-white/80 px-3 py-1 text-xs font-medium text-slate-700 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200">{t(levelMeta.label)}</span>
                    ...
                    <GlassCard variant="soft" padding="md" hoverable={false} className="w-full max-w-md">
                      {dailyRecommendation.exercise ? (
                        <>
                          <div className="flex flex-wrap gap-2">
                            <DifficultyPill level={dailyRecommendation.exercise.difficulty} size="sm" label={t(getDifficultyMeta(dailyRecommendation.exercise.difficulty).label)} />
                          ...
                          <div className="mt-4 flex flex-col gap-3 sm:flex-row">
                            <Button variant="primary" size="sm" onClick={() => openExercise(dailyRecommendation.exercise!)}>{t(dailyRecommendation.ctaLabel)}</Button>
                            <Button variant="secondary" size="sm" onClick={() => { setCategory(dailyRecommendation.exercise!.category); setShowCompleted('incomplete'); }}>{t('切到对应题库')}</Button>
                          </div>
                        </>
                      ) : (
                ...
                <GlassCard variant="soft" padding="md" hoverable={false} className="mb-8">
                  ...
                  <div className="grid gap-3 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-5">
                    {LANGUAGE_SUPPORT.map((item) => (
                      <GlassCard key={item.id} variant="soft" padding="sm" hoverable={false}>
                ...
                <div className="mb-8 grid gap-6 lg:grid-cols-[1.15fr_0.85fr]">
                  <GlassCard variant="soft" padding="md" hoverable={false}>
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      ...
                      {recommendedExercise && (
                        <Button variant="primary" size="sm" onClick={() => openExercise(recommendedExercise)}>
                          {isEnglish ? 'Continue Practice' : '继续刷题'}
                        </Button>
                      )}
                    </div>
                    {recommendedExercise ? (
                          ...
                          return (
                            <>
                              <div className="flex flex-wrap gap-2">
                                ...
                                <DifficultyPill level={recommendedExercise.difficulty} size="sm" label={t(getDifficultyMeta(recommendedExercise.difficulty).label)} />
            ...
            <GlassCard variant="soft" padding="md" hoverable={false} className="mb-8">
              <div className="grid gap-3 lg:grid-cols-[1fr_auto]">
                ...
                {activeFilters > 0 && (
                  <button onClick={clearFilters} className="min-h-[48px] cursor-pointer rounded-2xl bg-rose-100 px-4 text-sm font-semibold text-rose-700 dark:bg-rose-900/30 dark:text-rose-300">
                    {t('清空筛选')} {activeFilters}
                  </button>
                )}
              </div>
                ...
                <div className="flex flex-col gap-4 border-t border-slate-100 pt-4 dark:border-slate-700 lg:flex-row lg:flex-wrap">
                  ...
                  <div>
                    ...
                    <Tabs
                      ...
                      value={showCompleted}
                      onChange={(id) => setShowCompleted(id)}
                      variant="pill"
                      size="sm"
                      ariaLabel={t('完成状态')}
                    />
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      <Button variant="primary" size="sm" onClick={() => recommendedExercise && openExercise(recommendedExercise)} disabled={!recommendedExercise}>{t('推荐下一题')}</Button>
                      <Button variant="secondary" size="sm" onClick={() => setShowCompleted('incomplete')}>{t('继续未完成')}</Button>
                      <button onClick={() => setFocusOnly((value) => !value)} className={`min-h-[40px] cursor-pointer rounded-xl px-4 py-2 text-sm font-semibold transition-colors ${focusOnly ? 'bg-amber-500 text-white' : 'bg-slate-100 text-slate-700 dark:bg-slate-700 dark:text-slate-200'}`}>{t('考试重点')}</button>
                    </div>
                  </div>
                </div>
              </div>
            ...
            {selectedExercise ? (
              <div className="animate-fadeIn">
                <Button variant="secondary" size="sm" className="mb-4" onClick={() => setSelectedExercise(null)}>
                  {t('返回题目列表')}
                </Button>
                ...
                {orderedExercises.map((exercise) => {
                  ...
                  return (
                    <GlassCard
                      ...
                      padding="md"
                      hoverable
                      className={`text-left flex flex-col min-h-[280px] ${completed ? 'border-emerald-200/70 bg-emerald-50/60 dark:border-emerald-800/50 dark:bg-emerald-900/20' : ''}`}
                    >
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        <div className="flex flex-wrap gap-2">
                          <DifficultyPill level={exercise.difficulty} size="sm" label={t(getDifficultyMeta(exercise.difficulty).label)} />
              ...
              <GlassCard variant="soft" padding="lg" hoverable={false} className="text-center text-slate-500 dark:text-slate-300">
                {isEnglish ? 'No problems match the current filters. Clear filters or switch categories.' : '当前筛选范围没有题目，建议清空筛选或切换专题。'}
              </GlassCard>
            )}
          </>
        )}
